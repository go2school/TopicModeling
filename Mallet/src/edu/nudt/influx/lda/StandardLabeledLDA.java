 ...
 * 同时，在考虑主题生成词的时候仅仅是由一个主题确定一个词，此类主要利用LDA进行训练，得到word-label的分布，并将其打印出来
 */
...
//	    }
	...
	//先将其转换为对应的一个个的文件，然后在利用TF-IDF进行处理
	...
	//以LDA的tag-word概率分布作为tag的文本表示，首先将大于设定阈值概率的词筛选出来作为feature存放到hash表中，以便最后写为arff文件
...
//				double word_prob_four = dcmFmt.format(word_prob);
	...
	//首先通过LDA计算出每一个主题的词分布，基于此分布通过设置阈值选择feature并将相应的feature存储到相应的链表中
	//然后对mallet文件进行处理，生产linearSVM的数据格式文本time
 ...
 */
	...
	public static void main(String[] args) throws IOException {
		InstanceList ilist = InstanceList.load(new File(
				"F:\\Experiment\\MLClassification\\mallet\\sf_all_stemmed_files.mallet"));
		InstanceList tag_ilist = InstanceList.load(new File(
				"F:\\Experiment\\MLClassification\\mallet\\sf_all_stemmed_tags.mallet"));
		int numTrainIterations = 20;
		int pr = 15;
//		int numTopWords =;
		int burnIn = 17;
