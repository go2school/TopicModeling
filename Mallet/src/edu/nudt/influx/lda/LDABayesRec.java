...
public class LDABayesRec implements Serializable {
	...
	public LDABayesRec(double alpha, double beta) {
		this.alpha = alpha;
		this.beta = beta;
	}
...
//				System.out.println(ilist.get(di).getName().toString());
	...
	public void getProjectTopics(InstanceList testIlist,int max_num_topic, String file){
		...
		int totalToken = testIlist.getDataAlphabet().size();
...
//					totalTopic[key]*=count;
		...
		for(int i=0;i<numLabels;i++){
			totalTopic[i] += Math.log((double)tokensPerTopicSamples[i]/totalToken);
		}
		
		double[] totalTopicTemp = totalTopic.clone();
		
		for(int i=0;i<max_num_topic;i++){
			double max_support = -10000;
			int position = -1;
			for(int j=0;j<numLabels;j++){
...
//		Iterator average_it2 = typeTopicCountsAverage[typeID].keySet().iterator();
		Iterator average_it2 = typeTopicCountsSamples[typeID].keySet().iterator();
...
//			double ratio = (average_topic_count*average_topic_count)/(total_average_count*tokensPerTopicSamples[average_key2]);
			double ratio = (typeTopicCountsSamples[typeID].get(average_key2))/(tokensPerTopicSamples[average_key2]);
...
//		String tab_name = "sf_word_tag_count";
		...
		InstanceList ilist = InstanceList.load(new File(
				"D:\\WT_Experiments\\recSys_LDATR\\mallet\\oh_train\\trainingFiles.mallet"));
		InstanceList tag_ilist = InstanceList.load(new File(
				"D:\\WT_Experiments\\recSys_LDATR\\mallet\\oh_train\\trainingTags.mallet"));
		int numTrainIterations = 1000;
		int pr = 250;
		int labelNum=8;
		int burnIn = 970;
		System.out.println("Data loaded.");
	    LDABayesRec lda = new LDABayesRec(2,0.5);
...
//        lda.printCountDB(driver, url, user, pswd, autocommit, tab_name);
//        String testMallet = "F:\\Experiment\\Experiments\\recSys_LDATR\\mallet\\x";
        String testMallet = "D:\\WT_Experiments\\recSys_LDATR\\mallet\\ourTestFiles";
        String  testLabel= "D:\\WT_Experiments\\recSys_LDATR\\results\\testLabels\\bayes1000\\";
        String wordLabelDistr="D:\\WT_Experiments\\recSys_LDATR\\results\\wordLabelDistr\\bayes1000\\";
...
//				"F:\\Experiment\\LDA\\Experiments\\MiSoftware_MSR09\\result\\topicLabels\\500\\sf.topic"));
//		lda.printDocumentTopics(new File(
//				"F:\\Experiment\\LDA\\Experiments\\MiSoftware_MSR09\\result\\topicLabels\\500\\sf.lda"));
