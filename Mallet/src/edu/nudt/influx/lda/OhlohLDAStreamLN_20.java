...
/*
 * change ArrayList to HashMap<Integer,Integer>[] which is indexed by document id or type id;
 * the first Integer is topic id, the second Integer is the count of the topic under 
 * the document or type
 * 在计算测试集中词的主题概率时，考虑训练集中(numIterations-burnIn)之间的词-主题的赋值综合均值情况
 * 同时，在考虑主题生成词的时候由多个主题确定词而不仅仅是一个主题确定一个词
 * 此类主要利用LDA进行训练，得到word-label的分布，并将其打印出来（下一步要实现依此进行自动标注）
 */
...
public class OhlohLDAStreamLN_20 implements Serializable {
	...
	public OhlohLDAStreamLN_20(double alpha, double beta) {
		this.alpha = alpha;
		this.beta = beta;
	}
...
//	    }
	...
	private void getTypeTopicCountsAverageForOne(int typeID){
		String word = ilist.getDataAlphabet().lookupObject(typeID).toString();
//		wl.setWordDes(word);
...
*/		Iterator average_it = typeTopicCountsAverage[typeID].keySet().iterator();
		...
		while(average_it2.hasNext()){
			...
			typeTopicCountsAverage[typeID].put(average_key2, ratio);
...
*/
	...
	public ArrayList<DocTopic> ChannalTopic() {
		...
		for (int di = 0; di < topics.length; di++) {
			...
			docLen = topics[di].length;
			for (int ti = 0; ti < numTopics; ti++)
				topicDist[ti] = ((double) (hp.getCountInteger(docTopicCounts[di],ti) + alpha) / (docLen + tAlpha));
 ...
 */
	...
	public static void main(String[] args) throws IOException { 
		InstanceList ilist = InstanceList.load(new File(
				"F:\\Experiment\\LDA\\OhlohExperimentFiles\\mallet\\2-3\\files1.mallet"));
		InstanceList tag_ilist = InstanceList.load(new File(
				"F:\\Experiment\\LDA\\OhlohExperimentFiles\\mallet\\2-3\\tags1.mallet"));
		int numTrainIterations = 100;
		int pr = 50;
		System.out.println("Data loaded.");
	    OhlohLDAStreamLN_20 lda = new OhlohLDAStreamLN_20(2,0.5);
//	    lda.printWordIDDB(ilist);
		lda.estimate(ilist, tag_ilist, numTrainIterations, pr, 0, null, new Randoms(),95); 
        lda.printLabelWordDistribution("F:\\Experiment\\LDA\\OhlohExperimentFiles\\labelworddistribution\\2-3\\label_word_distribution6.txt");
        lda.getTypeTopicCountsAverage();
        for(int i=0;i<5;i++){
        	InstanceList test_ilist = InstanceList.load(new File(
				"F:\\Experiment\\LDA\\OhlohExperimentFiles\\test\\testMallet\\"+i+".mallet"));
        	lda.getProjectTopics(test_ilist, 50,"F:\\Experiment\\LDA\\OhlohExperimentFiles\\test\\testLabel\\6_"+i+".txt"); 
        	lda.printTypeTopicCountsAverage("F:\\Experiment\\LDA\\OhlohExperimentFiles\\test\\labelworddistribution\\2-3\\6_average_label_word_distribution"+i+".txt",test_ilist);
        }
//	    lda.printTopWords(numTrainTopWords, true);
//		lda.printToFile(30, true, new File(
//				"F:\\Experiment\\LDA\\OhlohExperimentFiles\\labelworddistribution\\ohloh6.topic"));
//		lda.printDocumentTopics(new File(
//				"F:\\Experiment\\LDA\\OhlohExperimentFiles\\labelworddistribution\\ohloh6.lda"));
